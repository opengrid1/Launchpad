      ...
      <div className="mx-auto flex h-16 max-w-5xl items-center gap-4 px-4 sm:gap-8 sm:px-6">
        ...
        <div className="ml-auto shrink-0">
          {isConnected && address ? (
            <button
              onClick={() => disconnect()}
              className="tnum flex h-9 items-center gap-1.5 rounded-full border border-edge-2 bg-panel px-3 text-xs font-medium text-ink transition-colors hover:border-accent hover:text-accent sm:h-10 sm:px-4 sm:text-sm"
              title="Disconnect"
            >
            ...
            <button
              ...
              disabled={isPending}
              className="h-10 rounded-full bg-accent px-4 text-sm font-semibold text-black transition-colors hover:bg-accent-2 disabled:opacity-60"
            >
