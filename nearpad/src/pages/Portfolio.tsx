...
export default function Portfolio() {
  ...
  return (
    <main>
      <div className="sec-h"><h2>Your portfolio</h2><span className="eyebrow">{accountId}</span></div>
      {owed.length > 0 && (
        <div className="card" style={{ marginBottom: 14 }}>
          ...
          <div className="card-b" style={{ display: "grid", gap: 10 }}>
            {owed.map((r) => {
              ...
              return (
                <div key={r.coin.account_id} className="row-flex" style={{ justifyContent: "space-between", alignItems: "center" }}>
                  <span><b>{r.coin.symbol}</b> <span className="faint">{v.fmt(r.h.claimable_dividends)} dividends · {v.fmt(r.h.credit)} credits</span></span>
      ...
      {isLoading && !rows ? <div className="skel" style={{ height: 240 }} /> : (
        <div className="list">
          <div className="rowh"><span>Coin</span><span>Pays in</span><span /><span className="r">Balance</span><span className="r">Value</span><span className="r">Owed</span></div>
          {(rows ?? []).length === 0 && <div className="empty">You do not hold any coin yet.</div>}
          {(rows ?? []).map((r) => {
            const v = val(r.coin.info.pair);
            const bal = units(r.h.balance, 18);
            ...
            return (
              <Link key={r.coin.account_id} to={`/t/${r.coin.account_id}`} className="rowc" style={{ color: "inherit" }}>
                <div className="coin"><Art src={r.coin.info.icon ?? undefined} name={r.coin.name} className="art" /><div style={{ minWidth: 0 }}><b>{r.coin.name}</b><small><span>{r.coin.symbol}</span>{r.coin.info.fee_wallet === accountId && <span className="chip official" style={{ height: 20, fontSize: 11 }}>Creator</span>}</small></div></div>
                <div className="c-hide-sm"><span className="chip">{pairSymbol(r.coin.info.pair)}</span></div>
                <div className="c-hide-sm c-hide-md" />
                <div className="r"><span>{unitsFmt(bal)}</span><small>{r.coin.symbol}</small></div>
                <div className="r c-hide-sm"><span>{v.fmt(value.toString(), true)}</span><small>{r.coin.info.phase === "Pool" ? "pool" : "curve"} price</small></div>
                <div className="r"><span className="vi">{v.fmt(owedRaw)}</span><small>claimable</small></div>
              </Link>
            );
          })}
        </div>
