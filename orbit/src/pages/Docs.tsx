    ...
    <main className="page">
      <section className="hero" style={{ gridTemplateColumns: "1fr", paddingBottom: 6 }}>
        <div>
          ...
          <p className="sub">One factory contract on HyperEVM enforces all of it. Two ways to go live, one fee, liquidity that never leaves.</p>
        </div>
      </section>
      ...
      <div className="rule"><b>{FEES.poolPct}%</b><div><h3>One fee</h3><p>Every buy and sell pays {FEES.poolPct}% into the pool. Fees collect there and are split on-chain whenever anyone presses Harvest: {FEES.creatorPct}% to the creator, {FEES.platformPct}% to the station. Paid in HYPE.</p></div></div>
      <div className="rule"><b>∞</b><div><h3>Liquidity locked</h3><p>The pool position is held by the factory. Creators can never pull it. The station can, and says so here: the owner can collect liquidity, and while an auction runs the owner can take HYPE out of escrow (spent HYPE, or everything). Nothing about that is hidden; every call is on-chain and shown on the coin's page.</p></div></div>
      ...
      <section className="sec">
        ...
        <p className="small" style={{ marginTop: 14 }}>Your bids, claims and coins live in your <Link to="/me" style={{ color: "var(--green)" }}>Studio</Link>.</p>
      </section>
    </main>
  );
}
