...
import { client, onair, type PairInfo } from "../lib/client";
...
// =====================================================================
const NATIVE_PAIR = (usd: number): PairInfo => ({ address: "0x5555555555555555555555555555555555555555", symbol: "HYPE", decimals: 18, usd, isNative: true });
const fmtPair = (v: bigint, p: PairInfo, d = 3) => (p.isNative ? hype(Number(formatUnits(v, 18)), d) : num(Number(formatUnits(v, p.decimals)), 4));

function MarketPage({ t }: { t: Token }) {
  const { data: hypeUsd = 0 } = useHypeUsd();
  const pair: PairInfo = t.pair ?? NATIVE_PAIR(hypeUsd);
  const pairUsd = pair.isNative ? hypeUsd || pair.usd : pair.usd;
  ...
  return (
    <main className="page">
      <div className="prog">
        ...
        <div>
          <h1>{t.name}</h1>
          <div className="meta"><span className="onair"><span className="dot" style={{ width: 6, height: 6, boxShadow: "none" }} />TRADING</span>{t.mode === "auction" && <span className="tagl auc">AUCTIONED</span>}<span className="mono">{t.symbol}</span><span>pairs {pair.symbol}</span><span>by <a href={`${env.explorerUrl}/address/${t.creator}`} target="_blank" rel="noreferrer" style={{ color: "var(--green)" }}>{short(t.creator)}</a></span><span>since {dateShort(t.createdAt)}</span></div>
        </div>
        <div className="price">
      ...
      <div className="two">
        <div>
          <div className="panel" style={{ padding: 12 }}>
            ...
            {candles && candles.length > 1 ? <Chart candles={candles} hypeUsd={pairUsd * 10 ** (18 - (pair.isNative ? 18 : pair.decimals))} mode={view} /> : <div className="chart" style={{ display: "grid", placeItems: "center", color: "var(--ink3)" }}>{candles ? "Not enough trades for a chart yet. The first buy starts it." : "Loading chart…"}</div>}
          </div>
          ...
          {tab === "trades" && <Trades address={t.address} symbol={t.symbol} pair={pair} pairUsd={pairUsd} />}
        ...
        <aside>
          <div className="panel desk" id="trade">
            <TradePanel token={t.address} symbol={t.symbol} priceWei={BigInt(t.priceWei || "0")} pair={pair} pairUsd={pairUsd} />
          </div>
          ...
          <div className="panel" style={{ marginTop: 14 }}>
            ...
            <dl className="specs">
              ...
              <dt>Liquidity</dt><dd>{usd(wei(t.liquidityWei) * pairUsd, { compact: true })}</dd>
              <dt>Volume · 24h</dt><dd>{usd(wei(t.volume24hWei) * pairUsd, { compact: true })}</dd>
              <dt>Pair</dt><dd>{pair.isNative ? "HYPE" : <Copy value={pair.address} label={pair.symbol} />}</dd>
              <dt>Trades · 24h</dt><dd>{num(t.txCount24h, 0)}</dd>
              <dt>Holders</dt><dd>{num(t.holderCount, 0)}</dd>
              <dt>Supply</dt><dd>1B fixed</dd>
              <dt>Format</dt><dd>{t.mode === "auction" ? "Auction" : "Instant"}{t.home !== "v2" ? ` · ${t.home} factory` : ""}</dd>
      ...
      {sheet && (
        <>
          ...
          <div className="sheet desk">
            <div className="grab" />
            <TradePanel token={t.address} symbol={t.symbol} priceWei={BigInt(t.priceWei || "0")} pair={pair} pairUsd={pairUsd} initial={sheet} />
          </div>
        </>
      )}
    </main>
...
// ---------- trade ----------
function TradePanel({ token, symbol, priceWei, pair, pairUsd, initial = "buy" }: { token: Address; symbol: string; priceWei: bigint; pair: PairInfo; pairUsd: number; initial?: "buy" | "sell" }) {
  const { address: me, isConnected } = useAccount();
  const qc = useQueryClient();
  const [side, setSide] = useState<"buy" | "sell">(initial);
  const [amt, setAmt] = useState("");
  const { data: bal } = useBalances(me, token, pair.isNative ? undefined : pair.address);
  const pd = pair.isNative ? 18 : pair.decimals;
  // Buys are entered in the pair asset (its own decimals), sells in the coin.
  const amountWei = useMemo(() => { try { return amt && Number(amt) > 0 ? (side === "buy" ? parseUnits(amt as `${number}`, pd) : parseEther(amt as `${number}`)) : 0n; } catch { return 0n; } }, [amt, side, pd]);
  ...
  // priceWei is pair units (its own decimals) per whole coin.
  ...
  const outNum = side === "buy" ? wei(out) : Number(formatUnits(out, pd));
  const impact = spot > 0n && sim != null ? (1 - Number(sim) / Number(spot)) * 100 : null;
  const payBal = bal ? (pair.isNative ? bal.native : bal.pair) : 0n;

  const max = () => {
    if (!bal) return;
    if (side === "buy") { const keep = pair.isNative ? parseEther("0.01") : 0n; setAmt(formatUnits(payBal > keep ? payBal - keep : 0n, pd)); }
  ...
  const buyChips = pair.isNative ? [0.1, 0.5, 1, 5] : [0.01, 0.05, 0.1, 0.5];
  ...
  return (
    <>
      ...
      <div className="amount">
        <div className="lbl"><span>{side === "buy" ? "You pay" : "You sell"}</span><span>{bal ? (side === "buy" ? `${fmtPair(payBal, pair)} ${pair.symbol}` : `${num(wei(bal.token))} ${symbol}`) : ""}</span></div>
        <div className="in"><input inputMode="decimal" placeholder="0" value={amt} onChange={(e) => setAmt(e.target.value.replace(/[^0-9.]/g, ""))} /><span className="unit">{side === "buy" ? pair.symbol : symbol}</span></div>
      </div>
      <div className="chips">
      ...
      <dl className="quote">
        <dt>You get</dt><dd>{amountWei > 0n ? `${side === "buy" ? num(outNum) : pair.isNative ? hype(outNum) : num(outNum, 4)} ${side === "buy" ? symbol : pair.symbol}` : "—"}</dd>
        <dt>Value</dt><dd>{amountWei > 0n ? usd(side === "buy" ? Number(formatUnits(amountWei, pd)) * pairUsd : outNum * pairUsd) : "—"}</dd>
      ...
      <p className="note">{pair.isNative ? "" : `This coin trades against ${pair.symbol}: buys spend it (approve once), sells pay out in it. Get ${pair.symbol} on Hyperliquid spot and transfer it to HyperEVM. `}Slippage 5%, capped to the real fill so the trade cannot revert on the floor. Trades settle on HyperSwap.</p>
    </>
  );
}
    ...
    <div className="panel pay" style={{ marginTop: 14 }}>
      <div className="between">
        <div>
          ...
          <div className="v">{hype(claimable, 5)} {pair.symbol}</div>
        </div>
...
// ---------- trades / holders ----------
function Trades({ address, symbol, pair, pairUsd }: { address: Address; symbol: string; pair: PairInfo; pairUsd: number }) {
  // Trade amounts come in the pair's own units.
  const q = (v: string) => { const raw = BigInt(v); return { n: Number(formatUnits(raw, pair.isNative ? 18 : pair.decimals)), s: fmtPair(raw, pair) }; };
    ...
    <div className="log">
      ...
      {trades.map((tr) => (
        <a key={tr.id} className="li" href={`${env.explorerUrl}/tx/${tr.txHash}`} target="_blank" rel="noreferrer">
          ...
          <span className="r">{num(wei(tr.tokenAmount))} {symbol}<small>{q(tr.nativeAmountWei).s} {pair.symbol} · {usd(q(tr.nativeAmountWei).n * pairUsd)}</small></span>
        </a>
      ))}
    </div>
  );
