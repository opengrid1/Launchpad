            ...
            {data.created.map((t) => {
              ...
              return (
                <div key={t.address} className="li">
                  <Art src={t.metadata?.logo} name={t.name} className="art" size={44} />
                  <div><Link to={`/t/${t.address}`} style={{ color: "inherit", fontWeight: 600 }}>{t.name}</Link><div className="l2">{t.symbol} · {live ? `auction · ${hype(wei(t.auction!.committed), 2)} HYPE committed` : `${usd(t.marketCapUsd, { compact: true })} cap · ${usd(wei(t.volume24hWei) * (t.pair?.usd ?? hypeUsd), { compact: true })} today`}</div></div>
                  <div className="r"><div className="l2">{t.mode === "auction" ? "auction" : "instant"} · pairs {t.pair?.symbol ?? "HYPE"} · fee {FEES.creatorPct}%</div></div>
