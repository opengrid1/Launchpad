...
export default function Me() {
  ...
  if (!isConnected || !me) return <main className="page"><section className="hero"><h1>Your <em>coins</em>.</h1><p className="sub">Connect a wallet to see what you hold and the coins you launched.</p><div className="cta"><button className="btn acc" onClick={() => openWalletModal()}>Connect wallet</button></div></section></main>;

  const value = data?.held.reduce((s, h) => s + wei(h.bal) * Number(h.t.priceUsd), 0) ?? 0;
  const earned = data ? [...data.rewards.entries()].reduce((s, [addr, r]) => { const t = tokens?.find((x) => x.address.toLowerCase() === addr); return s + (r && t && r.isCreator ? wei(r.creatorFees) * t.pair.usd : 0); }, 0) : 0;
  ...
  return (
    <main className="page">
      <section className="lead">
        <div className="say"><h1>Your <em>coins</em>.</h1><p>{short(me)}. Holdings and the coins you launched.</p></div>
        <div className="stats">
          <div className="acc"><b>{usd(value, { compact: true })}</b><span>holdings</span></div>
          <div><b>{usd(earned, { compact: true })}</b><span>creator fees to collect</span></div>
        ...
        {!data ? <div className="skeleton" style={{ height: 120 }} /> : data.held.length === 0 ? <div className="panel"><div className="empty">Nothing held yet. <Link to="/" className="acc">Browse coins</Link></div></div> : (
          <>
            <div className="tbl"><table>
              <thead><tr><th>Coin</th><th className="num">Balance</th><th className="num">Price</th><th className="num">Value</th><th></th></tr></thead>
              <tbody>{data.held.map(({ t, bal }) => (
                <tr key={t.address}>
            ...
            <div className="rows">
              {data.held.map(({ t, bal }) => (
                <Link key={t.address} to={`/t/${t.address}`}>
                  ...
                  <div><div className="l1">{t.name} <span className="faint">{t.symbol}</span></div><div className="l2">{num(wei(bal))} · {usd(t.priceUsd)} each</div></div>
      ...
      <section className="sec">
        ...
        {!data ? <div className="skeleton" style={{ height: 120 }} /> : data.created.length === 0 ? <div className="panel"><div className="empty">Nothing yet. <Link to="/launch" className="acc">Launch a coin</Link></div></div> : (
          <>
            <div className="tbl"><table>
              <thead><tr><th>Coin</th><th className="num">Market cap</th><th className="num">Vol 24h</th><th className="num">Fees to collect</th><th className="num">Lifetime</th><th></th></tr></thead>
              <tbody>{data.created.map((t) => { const r = data.rewards.get(t.address.toLowerCase()); const fees = r?.creatorFees ?? 0n; return (
                <tr key={t.address}>
                  <td><Link to={`/t/${t.address}`} className="coin"><Art src={t.metadata?.logo} name={t.name} className="art" /><b>{t.name}</b><small>{t.symbol}</small></Link></td>
                  <td className="num">{usd(t.marketCapUsd, { compact: true })}</td>
                  <td className="num">{usd(wei(t.volume24hWei) * t.pair.usd, { compact: true })}</td>
                  <td className={"num " + (fees > 0n ? "up" : "faint")}>{usd(wei(fees))}</td>
                  <td className="num dim">{usd(wei(r?.totalCreator ?? 0n))}</td>
                  <td className="num"><button className="btn acc sm" disabled={fees === 0n} onClick={claim("Collect creator fees", () => client.claimCreatorFees(t.address, false))}>Collect</button></td>
                </tr>); })}</tbody>
            </table></div>
            <div className="rows">
              {data.created.map((t) => { const r = data.rewards.get(t.address.toLowerCase()); const fees = r?.creatorFees ?? 0n; return (
                <a key={t.address} href="#" onClick={(e) => { e.preventDefault(); if (fees > 0n) claim("Collect creator fees", () => client.claimCreatorFees(t.address, false))(); }}>
                  <Art src={t.metadata?.logo} name={t.name} className="art" />
                  <div><div className="l1">{t.name} <span className="faint">{t.symbol}</span></div><div className="l2">{usd(t.marketCapUsd, { compact: true })} cap · {usd(wei(t.volume24hWei), { compact: true })} vol</div></div>
                  <div className="r"><span className={fees > 0n ? "up" : "faint"}>{usd(wei(fees))}</span><small>{fees > 0n ? "tap to collect" : "no fees yet"}</small></div>
                </a>); })}
            </div>
            <p className="note">Your {FEES.creatorPct}% of every pool fee accrues in USDC. Collect whenever you like.</p>
          </>
        )}
      </section>
    </main>
