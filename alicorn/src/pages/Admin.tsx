    ...
    <main>
      ...
      <section className="sec two">
        <div className="pane pad">
          <h3>Launches</h3>
          <div style={{ marginTop: 10 }}>{cfg?.paused ? <button className="b vio" onClick={call("Resume launches", "resume")}>Resume launches</button> : <button className="b ghost" onClick={call("Pause launches", "pause")}>Pause launches</button>}</div>
          <p className="note">Pausing blocks new launches only. Trading, rewards and claims keep running.</p>
        </div>
        <div className="pane pad">
          <h3>Fee recipient</h3>
          <div className="row-flex" style={{ marginTop: 10 }}><input className="in" style={{ flex: 1 }} placeholder={cfg?.feeRecipient ?? "0x…"} value={f.feeTo} onChange={set("feeTo")} /><button className="b ghost" disabled={!/^0x[0-9a-fA-F]{40}$/.test(f.feeTo)} onClick={call("Set fee recipient", "setFeeRecipient", [f.feeTo as Address])}>Set</button></div>
    ...
    <>
      ...
      <div className="pane pad" style={{ marginTop: 12 }}>
        ...
        <div className="row-flex">
          ...
          <button className="b vio" disabled={!isAddr(add.address) || (!(Number(add.usd) > 0) && !isAddr(add.feed))} onClick={call("Approve pair asset", "setQuoteAsset", [add.address as Address, true, usd8(add.usd), (isAddr(add.feed) ? add.feed : ZERO) as Address])}>Approve</button>
        </div>
...
function Fees({ tokens, call }: { tokens?: Token[]; call: (label: string, fn: Fn, args?: unknown[]) => () => Promise<void> }) {
  ...
  return (
    <>
      <div className="sec-h">
        <h2>Coins</h2>
        <div className="row-flex"><span className="eyebrow">{withFees.length} coin{withFees.length === 1 ? "" : "s"} · {usd(totalUsd, { compact: true })} waiting</span><button className="b pri" disabled={withFees.length === 0} onClick={collect(`Collect fees from ${withFees.length} coins`, withFees)}>Collect all fees</button></div>
      </div>
