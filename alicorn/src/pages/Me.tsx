...
/** Your statement: what every coin you hold has paid you, and what you can claim. */
export default function Me() {
  ...
  if (!isConnected || !me) return <main className="gate"><h1>Your <em>statement</em>.</h1><p>Connect a wallet to see what every coin you hold has paid you, and claim it.</p><button className="b vio" onClick={() => openWalletModal()}>Connect wallet</button></main>;
  ...
  return (
    <main>
      <div className="eyebrow mono">{me}</div>
      <h1 style={{ marginTop: 12 }}>Your <em>statement</em>.</h1>
      <div className="stats-line">
        <div><span className="eyebrow">Claimable now</span><b className="vi">{usd(owed, { compact: true })}</b></div>
        <div><span className="eyebrow">Holdings</span><b>{usd(value, { compact: true })}</b></div>
        <div><span className="eyebrow">Coins held</span><b>{data?.held.length ?? 0}</b></div>
        <div><span className="eyebrow">Launched</span><b>{data?.created.length ?? 0}</b></div>
      </div>

      <section className="sec">
        <div className="sec-h"><h2>Held</h2><span className="eyebrow">paid in the pair asset, per trade</span></div>
        {!data ? <div className="skel" style={{ height: 120 }} /> : data.held.length === 0 ? <div className="pane"><div className="empty">Nothing held yet. <Link to="/" className="vi">See the board</Link></div></div> : (
          <div className="tbl"><table>
            <thead><tr><th>Coin</th><th>Pays in</th><th className="num">Balance</th><th className="num">Value</th><th className="num">Owed to you</th><th></th></tr></thead>
            <tbody>{data.held.map(({ t, bal }) => { const r = data.rewards.get(t.address.toLowerCase()); const p = r?.pending ?? 0n; const eth = t.pair.ethRoute && !t.pair.isNative; return (
              <tr key={t.address}>
                ...
                <td className="num">{p > 0n ? <button className="b vio sm" onClick={claim("Claim rewards", () => client.claimRewards(t.address, eth))}>Claim{eth ? " as ETH" : ""}</button> : <Link className="b ghost sm" to={`/t/${t.address}`}>Trade</Link>}</td>
              </tr>); })}</tbody>
          </table></div>
        )}
      </section>
      ...
      <section className="sec">
        ...
        {!data ? <div className="skel" style={{ height: 120 }} /> : data.created.length === 0 ? <div className="pane"><div className="empty">Nothing yet. <Link to="/launch" className="vi">Launch a coin</Link></div></div> : (
          <div className="tbl"><table>
            <thead><tr><th>Coin</th><th>Pays in</th><th className="num">Market cap</th><th className="num">Vol 24h</th><th className="num">Paid to holders</th><th className="num">Your fees</th><th className="num">Lifetime</th><th></th></tr></thead>
            <tbody>{data.created.map((t) => { const r = data.rewards.get(t.address.toLowerCase()); const fees = r?.creatorFees ?? 0n; const eth = t.pair.ethRoute && !t.pair.isNative; return (
              <tr key={t.address}>
                ...
                <td className="num"><button className="b vio sm" disabled={fees === 0n} onClick={claim("Claim creator fees", () => client.claimCreatorFees(t.address, eth))}>Claim{eth ? " as ETH" : ""}</button></td>
              </tr>); })}</tbody>
          </table></div>
        )}
      </section>
      <p className="note">Rewards are credited as each trade settles, in the coin's pair asset. Pairs with an ETH route claim straight as ETH.</p>
    </main>
  );
}
