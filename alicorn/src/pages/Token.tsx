...
function Coin({ t }: { t: Token }) {
  ...
  const kind = pair.isNative ? "eth" : isTokenPair(pair.address) || pair.v3Fee ? "token" : "stock";
  const meta = stockByAddress(pair.address);
  const fees = t.rewards ? t.rewards.holders + t.rewards.creator + t.rewards.platform : 0n;
  const pw = (x: bigint | string) => wei(x, pair.decimals);
        ...
        <div>
          <div className="card">
            <div className="strip">
              ...
              <div><span>Volume 24h</span><b>{usd(pw(t.volume24hWei) * pair.usd, { compact: true })}</b></div>
              <div><span>Trades 24h</span><b>{num(t.txCount24h, 0)}</b></div>
              <div><span>Holders</span><b>{num(t.holderCount, 0)}</b></div>
              <div><span>Paid to holders</span><b className="vi">{t.rewards ? `${num(pw(t.rewards.holders), 4)} ${pair.symbol}` : "—"}</b></div>
              <div><span>Last trade</span><b>{last ? <><span className={last.isBuy ? "up" : "down"}>{last.isBuy ? "Buy" : "Sell"}</span> {usd(pw(last.nativeAmountWei) * pair.usd)}</> : "—"}</b></div>
            </div>
            ...
            <div className="chart-b">{candles ? <Chart candles={candles} hypeUsd={pair.usd} mode={view} volumeUsd={pw(t.volume24hWei) * pair.usd} /> : <div className="gc-empty">Chart loads with the first trade.</div>}</div>
          </div>
          ...
          <div className="card" style={{ marginTop: 12 }}>
            ...
            <div className="card-b">
              <dl className="kv">
                ...
                <dt>ETH route</dt><dd>{pair.isNative ? "Native" : pair.v3Fee ? `ETH → ${pair.symbol} (V3 ${pair.v3Fee / 10_000}%)` : pair.ethRoute ? `ETH → ${meta?.route?.via === "WETH" ? "" : `${meta?.route?.via} → `}${pair.symbol}` : "None, pay in the asset"}</dd>
                {!pair.isNative && <><dt>Pair contract</dt><dd><Copy value={pair.address} label={pair.symbol} /></dd></>}
                <dt>Liquidity</dt><dd>{t.reserves ? <>{hype(pw(t.reserves.pair), 4)} {pair.symbol} · {usd(pw(t.reserves.pair) * pair.usd, { compact: true })}<span className="dim">{cnum(wei(t.reserves.token))} {t.symbol}</span></> : usd(pw(t.liquidityWei) * pair.usd, { compact: true })}</dd>
                <dt>Fees collected</dt><dd>{hype(pw(fees), 4)} {pair.symbol}<span className="dim">{FEES.taxPct}% per trade · {FEES.holderPct}/{FEES.creatorPct}/{FEES.platformPct} split</span></dd>
...
function Yours({ token, pair }: { token: Address; pair: PairInfo }) {
  ...
  const pw = (x: bigint) => wei(x, pair.decimals);
  ...
  return (
    <>
      ...
      <div className="card-b rw">
        ...
        <div className="big">{data ? hype(pw(data.pending), 5) : "—"}<small>{unit}</small></div>
        <div className="sub">{!me ? "Connect a wallet to see your balance and rewards." : data ? `${num(wei(data.balance))} held · ${usd(pw(data.pending) * pair.usd)} claimable` : "Loading…"}</div>
        ...
        {data?.isCreator && (
          <div className="creator">
            <span className="eyebrow">Creator fees · lifetime {hype(pw(data.totalCreator), 4)} {unit}</span>
            <div className="big" style={{ fontSize: 20 }}>{hype(pw(data.creatorFees), 5)}<small>{unit}</small></div>
...
function TradeBox({ token, symbol, priceWei, pair, ethUsd, initial = "buy" }: { token: Address; symbol: string; priceWei: bigint; pair: PairInfo; ethUsd: number; initial?: "buy" | "sell" }) {
  ...
  const payDec = payEth ? 18 : pair.decimals;
  const { data: bal } = useBalances(me, token, pair.isNative ? undefined : pair.address);
  const { data: fee } = useFeeNow(token);
  const amountWei = useMemo(() => { try { return amt && Number(amt) > 0 ? parseUnits(amt as `${number}`, side === "buy" ? payDec : 18) : 0n; } catch { return 0n; } }, [amt, side, payDec]);
  ...
  // priceWei is pair raw units per whole coin; a buy in ETH is first turned into pair units at k.
  const pairUnit = 10 ** pair.decimals;
  const spot = priceWei > 0n ? (side === "buy" ? BigInt(Math.floor((Number(amountWei) * k * (payEth ? pairUnit / 1e18 : 1) * 1e18) / Number(priceWei))) : BigInt(Math.floor((Number(amountWei) * Number(priceWei)) / 1e18 / k * (payEth ? 1e18 / pairUnit : 1)))) : 0n;
  const feeBps = fee?.total ?? FEES.taxPct * 100;
  const out = sim ?? (spot * BigInt(10_000 - feeBps)) / 10_000n;
  const outNum = wei(out, side === "buy" ? 18 : payDec);
  ...
  const max = () => { if (!bal) return; if (side === "buy") { const keep = payEth ? parseEther("0.005") : 0n; setAmt(formatUnits(payBal > keep ? payBal - keep : 0n, payDec)); } else setAmt(formatUnits(bal.token, 18)); };
  const chip = (f: number) => { if (side === "buy") setAmt(String(f)); else if (bal) setAmt(formatUnits((bal.token * BigInt(Math.round(f * 100))) / 100n, 18)); };
  const go = async () => {
  ...
  const holdersCut = amountWei > 0n ? wei(side === "buy" ? amountWei : out, payDec) * payUsd * (FEES.taxPct / 100) * (FEES.holderPct / 100) : 0;
  return (
    <>
      ...
      <div className="amount">
        <div className="lbl"><span>{side === "buy" ? "You pay" : "You sell"}</span><button type="button" onClick={max}>{bal ? (side === "buy" ? `${hype(wei(payBal, payDec), 4)} ${payUnit}` : `${num(wei(bal.token))} ${symbol}`) : "Max"}</button></div>
      ...
      <dl className="quote">
        ...
        <dt>Value</dt><dd><b>{amountWei > 0n ? usd(side === "buy" ? wei(amountWei, payDec) * payUsd : outNum * payUsd) : "—"}</b></dd>
    ...
    <div className="log">
      ...
      {slice.map((tr) => (
        <a key={tr.id} className="li" href={`${env.explorerUrl}/tx/${tr.txHash}`} target="_blank" rel="noreferrer">
          ...
          <span className="r">{num(wei(tr.tokenAmount))} {symbol}<small>{hype(wei(tr.nativeAmountWei, pair.decimals), 4)} {pair.symbol} · {usd(wei(tr.nativeAmountWei, pair.decimals) * pair.usd)}</small></span>
        </a>
      ))}
