...
// Alicorn Telegram bot. Watches the factory for new launches and the
...
const SITE = process.env.SITE_URL || "https://www.alicorn.fun";
...
function launchText(c) {
  const lines = [
    ...
    ``,
    `<a href="${SITE}/t/${c.token}">Trade on Alicorn</a> · <a href="https://etherscan.io/token/${c.token}">Etherscan</a>`,
  ];
...
function tradeText(c, isBuy, pairAmt, pairUsd, mcapUsd, trader, tx) {
  ...
  return [
    ...
    ``,
    `<a href="${SITE}/t/${c.token}">Trade on Alicorn</a>`,
  ].join("\n");
}
