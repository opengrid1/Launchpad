...
export type StockToken = TokenSummary & {
  ...
  poolId: Hex;
  launchBlock: number;
  /** Lifetime pair-asset credited to the burn reserve / creator / platform. */
  fees?: { burn: bigint; creator: bigint; platform: bigint };
  /** Burn ledger: pair waiting in the reserve, coins burned so far, pair spent on buybacks, and the trigger threshold. */
  burn?: { reserve: bigint; burned: bigint; spent: bigint; min: bigint };
  /** What actually sits in the pool right now: pair asset and coin, in wei. */
  reserves?: { pair: bigint; token: bigint };
};

/** A coin's fee ledger: the burn reserve, what has been burned, and the creator's and platform's claimable shares. */
export interface LedgerView {
  /** Pair asset waiting in the burn reserve. */
  reserve: bigint;
  /** Reserve size that triggers the automatic buyback. */
  buybackMin: bigint;
  /** Coins burned so far (wei of the coin). */
  burned: bigint;
  /** Pair asset spent on buybacks so far. */
  spent: bigint;
  creatorFees: bigint;
  platformFees: bigint;
  totalBurn: bigint;
 ...
 *  and the burn ledger straight from the factory, the V4 PoolManager and the coins;
...
export class StockPadClient {
  ...
  private async summarize(core: Core): Promise<StockToken> {
    ...
    // Fee totals and the burn ledger. Supply shrinks with every buyback, so the
    // market cap uses the live supply, not the launch supply.
    let fees: StockToken["fees"];
    ...
    return {
      ...
      createdAt: core.createdAt, featured: false, metadata: core.metadata as any, totalSupply: supply.toString(),
      priceWei: priceWei.toString(), priceUsd: String(priceUsd), marketCapUsd: String(mcapLive || mcap), liquidityWei: liquidityWei.toString(),
      ...
      pair, poolId: core.poolId, launchBlock: Number(core.launchBlock), fees, reserves, burn,
    };
  }
  ...
  async ledger(token: Address, account?: Address): Promise<LedgerView> {
    ...
    const [reserve, buybackMin, burned, spent, creatorFees, platformFees, tb, tc, tp, creator, balance] = (await this.pc.multicall({ allowFailure: false, contracts: [
      ...
      { address: token, abi: tokenAbi, functionName: "totalBurnFees" },
  ...
  async buybackAndBurn(token: Address): Promise<Hex> {
    const wc = this.wallet();
    return wc.writeContract({ address: token, abi: tokenAbi, functionName: "buybackAndBurn", chain: wc.chain, account: wc.account! });
  }
  ...
  async config(): Promise<ConfigView> {
    const [admin, owner, feeRecipient, paused, taxBps, creatorBps, burnBps, converter, total] = (await this.pc.multicall({ allowFailure: false, contracts: [
      ...
      { address: ADDRESSES.factory, abi: factoryAbi, functionName: "BURN_BPS" },
    ...
    return { admin, owner, feeRecipient, paused, taxBps: Number(taxBps), creatorBps: Number(creatorBps), burnBps: Number(burnBps), ethUsd: await this.ethUsd(), converter, totalTokens: Number(total) };
  }
  ...
  /** Burn reserves waiting in each coin (pair wei), for the admin's "burn all" button. */
  async burnWaiting(tokens: Address[]): Promise<Map<string, bigint>> {
    if (tokens.length === 0) return new Map();
    const res = await this.pc.multicall({ allowFailure: true, contracts: tokens.map((t) => ({ address: t, abi: tokenAbi, functionName: "burnReserve" as const })) });
    return new Map(tokens.map((t, i) => [t.toLowerCase(), res[i].status === "success" ? (res[i].result as bigint) : 0n]));
  }

  /** Buy back and burn many coins in one transaction via Multicall3 (buybackAndBurn is permissionless). */
  async burnMany(coins: Address[]): Promise<Hex> {
    const wc = this.wallet();
    const calls = coins.map((c) => ({ target: c, allowFailure: true, callData: encodeFunctionData({ abi: tokenAbi, functionName: "buybackAndBurn" }) }));
