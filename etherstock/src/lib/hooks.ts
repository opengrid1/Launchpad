...
export function useLedger(token?: Address, account?: Address) {
  return useQuery({ queryKey: ["ledger", token?.toLowerCase(), account?.toLowerCase()], queryFn: () => client.ledger(token!, account), enabled: !!token, refetchInterval: 30_000 });
}
