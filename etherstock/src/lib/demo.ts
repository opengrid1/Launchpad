...
import { StockPadClient, type ConfigView, type PairInfo, type QuoteView, type LedgerView, type StockToken } from "./client";
...
interface Seed { name: string; symbol: string; pair: PairInfo; mcap: number; chg: number; holders: number; ageH: number; vol: number; burnedPct: number; desc: string; logo?: string; seed: number }
const SEEDS: Seed[] = [
  { name: "Nvidia Enjoyer", symbol: "NVJ", pair: stockPair("NVDAon"), mcap: 48_200, chg: 38.4, holders: 212, ageH: 5, vol: 61_400, burnedPct: 2.4, desc: "Jensen said the supercycle is not over. We agree. Paired with NVDAon so every buyback is paid in NVIDIA.", seed: 3 },
  { name: "Tesla Dad", symbol: "TDAD", pair: stockPair("TSLAon"), mcap: 21_900, chg: -12.1, holders: 88, ageH: 19, vol: 18_700, burnedPct: 1.1, desc: "Buying the dip since 2019.", seed: 7 },
  { name: "Moon Cat", symbol: "MCAT", pair: ethPair, mcap: 9_400, chg: 6.8, holders: 41, ageH: 2, vol: 5_100, burnedPct: 0.6, desc: "The cat that burns itself to feed you.", seed: 11 },
  { name: "Index Fund Andy", symbol: "ANDY", pair: stockPair("SPYon"), mcap: 132_000, chg: 4.2, holders: 640, ageH: 72, vol: 44_000, burnedPct: 5.2, desc: "Boring on purpose. Buybacks paid in SPY, forever.", seed: 5 },
  { name: "Apple Bottom", symbol: "APLB", pair: stockPair("AAPLon"), mcap: 3_100, chg: null as unknown as number, holders: 3, ageH: 0.2, vol: 300, burnedPct: 0, desc: "Just launched.", seed: 9 },
  { name: "Silver Surfer", symbol: "SURF", pair: stockPair("SLVon"), mcap: 15_600, chg: 21.0, holders: 120, ageH: 30, vol: 12_200, burnedPct: 1.8, desc: "Precious metal, precious memes.", seed: 13 },
];
...
function build(s: Seed, i: number): StockToken {
  ...
  return {
    ...
    createdAt: NOW - Math.round(s.ageH * 3600), featured: false, metadata: { description: s.desc, logo: s.logo, twitter: "https://x.com/Etherstock_" },
    ...
    fees: { burn: toWei(paid * 0.3), creator: toWei(paid * 0.5), platform: toWei(paid * 0.2) },
    burn: { reserve: toWei(paid * 0.3 * 0.15), burned: toWei(SUPPLY * s.burnedPct / 100), spent: toWei(paid * 0.3 * 0.85), min: toWei(25 / s.pair.usd) },
  };
}
...
export class DemoClient extends StockPadClient {
  ...
  override async ledger(token: Address): Promise<LedgerView> {
    const t = this.find(token); const f = t?.fees ?? { burn: 0n, creator: 0n, platform: 0n }; const b = t?.burn ?? { reserve: 0n, burned: 0n, spent: 0n, min: 0n };
    return { reserve: b.reserve, buybackMin: b.min, burned: b.burned, spent: b.spent, creatorFees: 0n, platformFees: f.platform / 5n, totalBurn: f.burn, totalCreator: f.creator, totalPlatform: f.platform, isCreator: false, balance: 0n };
  }
  ...
  override async config(): Promise<ConfigView> {
    return { admin: ADMIN, owner: "0x0000000000000000000000000000000000000000", feeRecipient: ADMIN, paused: false, taxBps: FEES.taxPct * 100, creatorBps: FEES.creatorPct * 100, burnBps: FEES.burnPct * 100, ethUsd: ETH_USD, converter: addrOf(0x70), totalTokens: TOKENS.length };
  }
  ...
  override async buybackAndBurn(): Promise<Hex> { return this.nope(); }
  override async burnMany(): Promise<Hex> { return this.nope(); }
  ...
  override async burnWaiting(tokens: Address[]): Promise<Map<string, bigint>> { return new Map(tokens.map((t) => [t.toLowerCase(), this.find(t)?.burn?.reserve ?? 0n])); }
  override async platformWaiting(tokens: Address[]): Promise<Map<string, bigint>> { return new Map(tokens.map((t) => [t.toLowerCase(), this.find(t)?.fees?.platform ?? 0n])); }
