...
type Sort = "new" | "mcap" | "vol" | "chg" | "burned" | "holders";
const SORTS: { k: Sort; l: string }[] = [{ k: "new", l: "New" }, { k: "mcap", l: "Market cap" }, { k: "vol", l: "Volume" }, { k: "chg", l: "24h" }, { k: "burned", l: "Burned" }, { k: "holders", l: "Holders" }];

const burnedPct = (t: Token) => (t.burn ? (Number(t.burn.burned) / 1e27) * 100 : 0);
const burnedUsd = (t: Token) => (t.burn ? wei(t.burn.spent) * t.pair.usd : 0);
...
export default function Home() {
  ...
  const totals = useMemo(() => {
    const t = tokens ?? [];
    return { n: t.length, vol: t.reduce((s, x) => s + wei(x.volume24hWei) * x.pair.usd, 0), burned: t.reduce((s, x) => s + burnedUsd(x), 0) };
  }, [tokens]);
  ...
  return (
    <main className="page">
      <section className="lead">
        <div className="say">
          <h1>Coins that <em>burn</em> themselves.</h1>
          <p>Launch a coin on Ethereum paired with ETH, SPY, NVIDIA or {stocks > 0 ? `any of ${stocks} tokenized stocks` : "another tokenized stock"}. Every trade pays {FEES.taxPct}%: half to the creator, {FEES.burnPct}% buys the coin back and burns it on the spot, the rest to the platform. Supply only goes down.</p>
        ...
        <div className="stats">
          ...
          <div className="ember"><b>{usd(totals.burned, { compact: true })}</b><span>bought back and burned</span></div>
        </div>
      </section>
  ...
  return (
    <Link to={`/t/${t.address}`} className={"tile" + (pinned ? " pinned" : "")} style={{ animationDelay: `${Math.min(i, 12) * 30}ms` }}>
      ...
      <div className="tile-meta">
        ...
        <span className="ember"><b>{burnedPct(t).toFixed(1)}%</b>burned</span>
