...
function Fees({ tokens, call }: { tokens?: Token[]; call: (label: string, fn: Fn, args?: unknown[]) => () => Promise<void> }) {
  ...
  const actions = (t: Token) => (
    <div className="row" style={{ justifyContent: "flex-end" }}>
      ...
      <button className="btn sm ember" disabled={reserve(t) === 0n} onClick={burn(`Burn ${t.symbol}`, [t])}>Burn now</button>
  ...
  return (
    <>
      <div className="sec-h">
        <h2>Coins</h2>
        <div className="tools">
          <span className="caps">{usd(totalUsd, { compact: true })} fees · {usd(reserveUsd, { compact: true })} to burn</span>
          ...
          <button className="btn ember" disabled={withReserve.length === 0} onClick={burn(`Burn ${withReserve.length} coins`, withReserve)}>Burn all</button>
