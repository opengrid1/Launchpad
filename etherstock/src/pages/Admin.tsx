...
import { ADDRESSES, FEES, isHidden } from "../lib/env";
  ...
  return (
    <main>
      ...
      <section className="sec">
        <div className="box"><dl className="kv">
          ...
          <dt>Fee</dt><dd>{cfg ? `${cfg.taxBps / 100}% · ${cfg.creatorBps / 100} / ${cfg.burnBps / 100} / ${(10000 - cfg.creatorBps - cfg.burnBps) / 100}` : "…"}</dd>
      ...
      <section className="sec two">
        <div className="box">
          <h3>Launches</h3>
          <div style={{ marginTop: 10 }}>{cfg?.paused ? <button className="b fire" onClick={call("Resume launches", "resume")}>Resume launches</button> : <button className="b" onClick={call("Pause launches", "pause")}>Pause launches</button>}</div>
          <p className="note">Pausing blocks new launches only. Trading, burns and claims keep running.</p>
        </div>
        <div className="box">
          <h3>Fee recipient</h3>
          <div className="row" style={{ marginTop: 10 }}><input className="in" style={{ flex: 1 }} placeholder={cfg?.feeRecipient ?? "0x…"} value={f.feeTo} onChange={set("feeTo")} /><button className="b" disabled={!/^0x[0-9a-fA-F]{40}$/.test(f.feeTo)} onClick={call("Set fee recipient", "setFeeRecipient", [f.feeTo as Address])}>Set</button></div>
      ...
      <p className="note"><Link to="/" className="ember">Back to the board</Link></p>
    </main>
  );
}
...
function Quotes({ call }: { call: (label: string, fn: Fn, args?: unknown[]) => () => Promise<void> }) {
  ...
  const row = (x: QuoteView) => (
    <tr key={x.address}>
      <td><b>{x.symbol}</b> <span className="faint">{x.name}</span> {!x.approved && <span className="chip">retired</span>} {x.ethRoute && !x.isNative && <span className="chip up">ETH route</span>}</td>
      <td><Copy value={x.address} /></td>
      <td className="num">{usd(x.usd)}</td>
      <td className="num dim">{x.liqUsd > 0 ? usd(x.liqUsd, { compact: true }) : "—"}</td>
      <td><div className="row"><input className="in" style={{ width: 96 }} inputMode="decimal" placeholder="USD" value={px[x.address] ?? ""} onChange={(e) => setPx({ ...px, [x.address]: e.target.value })} /><input className="in" style={{ width: 140 }} placeholder="feed (opt)" value={feed[x.address] ?? ""} onChange={(e) => setFeed({ ...feed, [x.address]: e.target.value })} /><button className="b sm" disabled={!(Number(px[x.address]) > 0) && !isAddr(feed[x.address])} onClick={call(`Price ${x.symbol}`, "setQuoteAsset", [x.address, true, usd8(px[x.address] ?? "0"), (isAddr(feed[x.address]) ? feed[x.address] : ZERO) as Address])}>{x.approved ? "Set" : "Approve"}</button>{x.approved && !x.isNative && <button className="b sm danger" onClick={call(`Retire ${x.symbol}`, "setQuoteAsset", [x.address, false, 0n, ZERO])}>Retire</button>}</div></td>
    </tr>
  );
  return (
    <>
      <div className="tools"><input className="in grow" placeholder="Filter pairs" value={q} onChange={(e) => setQ(e.target.value)} /></div>
      ...
      <div className="box" style={{ marginTop: 12 }}>
        <div className="lbl" style={{ marginBottom: 10 }}>Add a pair asset</div>
        <div className="row">
          ...
          <button className="b fire" disabled={!isAddr(add.address) || (!(Number(add.usd) > 0) && !isAddr(add.feed))} onClick={call("Approve pair asset", "setQuoteAsset", [add.address as Address, true, usd8(add.usd), (isAddr(add.feed) ? add.feed : ZERO) as Address])}>Approve</button>
        </div>
        <p className="note">A Chainlink feed, when set and fresh, overrides the USD price. The price sizes the $3k opening pool of every coin launched on that pair and the $25 burn threshold. Retiring stops new launches; existing coins keep trading. ETH cannot be retired.</p>
      </div>
    </>
  );
}

/** Every coin: platform fees waiting, burn reserves waiting, and admin-only
 *  liquidity recovery. */
function Fees({ tokens, call }: { tokens?: Token[]; call: (label: string, fn: Fn, args?: unknown[]) => () => Promise<void> }) {
  ...
  const { data: reserves } = useQuery({
    queryKey: ["burnWaiting", tokens?.map((t) => t.address).join(",")],
    enabled: !!tokens,
    refetchInterval: 30_000,
    queryFn: () => client.burnWaiting((tokens ?? []).map((t) => t.address)),
  });
  const pending = (t: Token) => waiting?.get(t.address.toLowerCase()) ?? 0n;
  const reserve = (t: Token) => reserves?.get(t.address.toLowerCase()) ?? 0n;
  const withFees = (tokens ?? []).filter((t) => pending(t) > 0n);
  const withReserve = (tokens ?? []).filter((t) => reserve(t) > 0n);
  const totalUsd = withFees.reduce((s, t) => s + wei(pending(t)) * t.pair.usd, 0);
  const reserveUsd = withReserve.reduce((s, t) => s + wei(reserve(t)) * t.pair.usd, 0);
  const refresh = async () => { await qc.invalidateQueries({ queryKey: ["platformWaiting"] }); await qc.invalidateQueries({ queryKey: ["burnWaiting"] }); await qc.invalidateQueries({ queryKey: ["tokens"] }); };
  ...
  const burn = (label: string, list: Token[]) => async () => {
    await ensureWallet();
    await runTx(label, () => (list.length === 1 ? client.buybackAndBurn(list[0].address) : client.burnMany(list.map((t) => t.address))), refresh);
  };
  const isAddr = (v: string) => /^0x[0-9a-fA-F]{40}$/.test(v);
  ...
  const actions = (t: Token) => (
    <div className="row" style={{ justifyContent: "flex-end" }}>
      <button className="b sm" disabled={pending(t) === 0n} onClick={collect(`Collect ${t.symbol} fees`, [t])}>Collect fees</button>
      <button className="b sm soft" disabled={reserve(t) === 0n} onClick={burn(`Burn ${t.symbol}`, [t])}>Burn now</button>
      <button className="b sm danger" onClick={() => setRecover({ t, pct: "100", to: me ?? "" })}>Recover LP</button>
    </div>
  );
  return (
    <>
      <div className="sec-h">
        <h2>Coins</h2>
        <div className="row">
          <span className="lbl">{usd(totalUsd, { compact: true })} fees · {usd(reserveUsd, { compact: true })} to burn</span>
          <button className="b pri" disabled={withFees.length === 0} onClick={collect(`Collect fees from ${withFees.length} coins`, withFees)}>Collect all fees</button>
          <button className="b fire" disabled={withReserve.length === 0} onClick={burn(`Burn ${withReserve.length} coins`, withReserve)}>Burn all</button>
          <button className="b danger" disabled={!tokens?.length} onClick={() => setRecoverAll({ pct: "100", to: me ?? "", busy: false, done: 0, total: tokens?.length ?? 0 })}>Recover all LP</button>
        </div>
      </div>
      <div className="box" style={{ marginBottom: 12 }}>
        <div className="lbl" style={{ marginBottom: 8 }}>Manage a coin by address</div>
        ...
        {found && (
          <div className="row" style={{ justifyContent: "space-between", marginTop: 12 }}>
            <Link to={`/t/${found.address}`} className="coin row"><Art src={found.metadata?.logo} name={found.name} className="art" /><span><b>{found.name}</b> <small className="faint">{found.symbol} · {usd(found.marketCapUsd, { compact: true })} · {hype(wei(pending(found)), 5)} {found.pair.symbol} waiting</small></span></Link>
            {actions(found)}
          </div>
        )}
      </div>
      {!tokens || !waiting ? <div className="skel" style={{ height: 120 }} /> : (
        <div className="tbl"><table>
          <thead><tr><th>Coin</th><th>Pair</th><th className="num">Market cap</th><th className="num">Fees waiting</th><th className="num">To burn</th><th className="num">Burned</th><th className="num">Actions</th></tr></thead>
          <tbody>{tokens.map((t) => { const w = pending(t); const r = reserve(t); const b = t.burn ? (Number(t.burn.burned) / 1e27) * 100 : 0; return (
            <tr key={t.address}>
              ...
              <td className={"num " + (r > 0n ? "ember" : "faint")}>{hype(wei(r), 5)} {t.pair.symbol}</td>
              <td className="num dim">{b.toFixed(2)}%</td>
              <td className="num">{actions(t)}</td>
            </tr>); })}</tbody>
        </table></div>
      )}
      <p className="note">Collect fees sends a coin's {FEES.platformPct}% platform share to the fee recipient; anyone can trigger it, the destination cannot change. Burn now spends whatever sits in a coin's burn reserve on the coin and burns it, ahead of the automatic trigger; anyone can trigger it, you only pay gas. Recover LP is admin-only: it pulls that share of the launch position (coins and pair) to a wallet you choose, and cannot be undone.</p>
      {recoverAll && (
        <div className="modal" onClick={() => !recoverAll.busy && setRecoverAll(null)}>
          <div className="box" onClick={(e) => e.stopPropagation()}>
            <h3>Recover liquidity from every coin</h3>
            <p className="note" style={{ marginTop: 6 }}>Pulls the chosen share of each coin's launch position (coins and pair asset) to one recipient. One transaction per coin: {tokens?.length ?? 0} confirmations, largest pools first. Not reversible, and trading on emptied pools stops working.</p>
            <div className="f" style={{ marginTop: 14 }}><label>Percent to remove (1–100)</label><input className="in" inputMode="decimal" value={recoverAll.pct} disabled={recoverAll.busy} onChange={(e) => setRecoverAll({ ...recoverAll, pct: e.target.value })} /></div>
            <div className="f"><label>Recipient</label><input className="in" value={recoverAll.to} disabled={recoverAll.busy} onChange={(e) => setRecoverAll({ ...recoverAll, to: e.target.value })} placeholder="0x…" spellCheck={false} /></div>
            ...
            <div className="row" style={{ marginTop: 14 }}>
              <button className="b" style={{ flex: 1 }} disabled={recoverAll.busy} onClick={() => setRecoverAll(null)}>Cancel</button>
              <button className="b danger" style={{ flex: 1 }} disabled={recoverAll.busy || !(Number(recoverAll.pct) > 0 && Number(recoverAll.pct) <= 100) || !isAddr(recoverAll.to.trim())} onClick={runRecoverAll}>Recover from {tokens?.length ?? 0} coins</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="modal" onClick={() => setRecover(null)}>
          <div className="box" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="row" style={{ marginTop: 16 }}>
              <button className="b" style={{ flex: 1 }} onClick={() => setRecover(null)}>Cancel</button>
