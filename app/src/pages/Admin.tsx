...
/**
 * Hidden owner-only console (route #/admin, not linked anywhere). "Withdraw LP" calls the
 * contract's `withdrawPosition`, which settles fees, removes all liquidity, and sends the
 * native HYPE + leftover tokens to the recipient in one transaction. Non-owners see a locked
 * screen; the on-chain `onlyOwner` guard is the real protection.
 */
export function AdminPage() {
  ...
  const withdraw = useCallback(
    async (token: Address, recipient: Address) => {
      ...
      try {
        ...
        push({ kind: 'info', title: 'Withdrawing LP…', txHash: hash })
        const receipt = await publicClient.waitForTransactionReceipt({ hash })
        push(
          receipt.status === 'success'
            ? { kind: 'success', title: 'LP withdrawn — HYPE + tokens sent', txHash: hash }
  ...
  return (
    <main className="mx-auto mt-10 max-w-2xl pb-10">
      ...
      <p className="mt-2 text-sm text-dim">Owner-only. Withdraw LP removes the pool's liquidity and sends the HYPE + tokens to the recipient.</p>
...
function ManualWithdraw({
  ...
  return (
    <section className="elev mt-6 rounded-2xl p-5 ring-1 ring-hair">
      ...
      <p className="mt-1 text-xs text-ghost">Sends the pool's HYPE + tokens to the recipient.</p>
...
function LaunchRowAdmin({
  ...
  return (
    <div className={`flex items-center justify-between gap-3 px-4 py-3 ${border ? 'border-t border-hair' : ''}`}>
      ...
      >
        {busy ? '…' : row.positionWithdrawn ? 'Withdrawn' : 'Withdraw LP'}
      </button>
    </div>
  )
}
