...
const LAUNCHPAD = '0xB00FE2F0EB0D7871D1c858963f59F2C6463e7F32'
...
function launchMessage(p: { name: string; symbol: string; creator: string; feeRecipient: string; nonce: number }) {
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const { name, symbol, tokenURI, creator, feeRecipient, nonce, signature } = body
    ...
    const valid = await verifyMessage({
      address: creator,
      message: launchMessage({ name, symbol, creator, feeRecipient: fr, nonce }),
      signature,
    })
    ...
    const hash = await wallet.writeContract({
      ...
      // creator_ = the user (relayer is authorised to launch on their behalf); fees route
      // to feeRecipient. Explicit gas: launches need ~6M, above the small-block estimate cap.
      args: [String(name).slice(0, 48), String(symbol).slice(0, 12).toUpperCase(), String(tokenURI), SUPPLY, creator, fr, 0n, 0n],
      gas: 24_000_000n,
    })
