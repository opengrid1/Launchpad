...
const RPC = process.env.ALCHEMY_HTTP || process.env.RPC_URL || "https://arb1.arbitrum.io/rpc";
const CHAIN_ID = Number(process.env.CHAIN_ID ?? "42161");
const EXPLORER = process.env.EXPLORER_URL || "https://arbiscan.io";
...
const SITE = process.env.SITE_URL || "https://www.arbistonk.fun";
const ONCE = process.argv.includes("--once");

const FACTORY = process.env.FACTORY || "0x0000000000000000000000000000000000000000";
const POOL_MANAGER = process.env.POOL_MANAGER || "0x360E68faCcca8cA495c1B759Fd9EEe466db9FB32";
const WETH = (process.env.WETH || "0x82aF49447D8a07e3bd95BD0d56f35241523fBab1").toLowerCase();
...
const p = new ethers.JsonRpcProvider(RPC, CHAIN_ID, { staticNetwork: true, batchMaxCount: 1 });
...
function launchText(c) {
  const lines = [
    ...
    ``,
    `<a href="${SITE}/t/${c.token}">Trade on Arbistonk</a> · <a href="${EXPLORER}/token/${c.token}">Arbiscan</a>`,
  ];
...
function tradeText(c, isBuy, pairAmt, pairUsd, mcapUsd, trader, tx) {
  ...
  return [
    ...
    `Trader: <a href="${EXPLORER}/address/${trader}">${short(trader)}</a> · <a href="${EXPLORER}/tx/${tx}">tx</a>`,
    ``,
    `<a href="${SITE}/t/${c.token}">Trade on Arbistonk</a>`,
  ].join("\n");
}
