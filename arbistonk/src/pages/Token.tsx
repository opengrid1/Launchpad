        ...
        <aside>
          ...
          <div className="panel">
            ...
            <dl className="kv">
              ...
              <dt>Links</dt><dd><a className="acc" href={`${env.explorerUrl}/token/${t.address}`} target="_blank" rel="noreferrer">Arbiscan</a> · <a className="acc" href={`https://dexscreener.com/${env.dexscreenerChain}/${t.poolId}`} target="_blank" rel="noreferrer">DexScreener</a></dd>
            </dl>
          </div>
        </aside>
      </div>
...
function Holders({ address, creator }: { address: Address; creator: Address }) {
  ...
  return (
    <>
      ...
      {holders.length > PAGE && <p className="note" style={{ textAlign: "center" }}>Top {PAGE} holders. Full list on <a className="acc" href={`${env.explorerUrl}/token/${address}#balances`} target="_blank" rel="noreferrer">Arbiscan</a>.</p>}
    </>
  );
}
