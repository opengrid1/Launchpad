...
function build(s: Seed, i: number): StockToken {
  ...
  return {
    ...
    createdAt: NOW - Math.round(s.ageH * 3600), featured: false, metadata: { description: s.desc, logo: s.logo, twitter: "https://x.com/arbistonk" },
...
export class DemoClient extends StockPadClient {
  ...
  private nope(): never { throw new Error("Preview build: the factory is not on Arbitrum yet, so nothing can be sent."); }
