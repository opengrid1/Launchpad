...
// deployments/alicorn-quotes.json; renounces factory ownership so only
...
async function main() {
  ...
  if (!factoryAddr) {
    const f = await (await ethers.getContractFactory("AlicornFactory")).deploy(deployer.address, admin, POOL_MANAGER, hookAddr, WETH, price, TAX_BPS, CREATOR_BPS, HOLDER_BPS);
  ...
  if (process.env.QUOTES !== "0") {
    ...
    for (const q of quotes) {
      ...
      try {
        const tx = await factory.setQuoteAsset(q.address, true, usd8, q.feed ?? ethers.ZeroAddress);
        await tx.wait();
  ...
  console.log(`  npx hardhat verify --network robinhood --contract contracts/v4/alicorn/AlicornFactory.sol:AlicornFactory ${factoryAddr} ${deployer.address} ${admin} ${POOL_MANAGER} ${hookAddr} ${WETH} ${price} ${TAX_BPS} ${CREATOR_BPS} ${HOLDER_BPS}`);
