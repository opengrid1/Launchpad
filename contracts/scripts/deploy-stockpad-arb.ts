...
// Deploys Arbistonk on Arbitrum One: the Stonkreum stockpad (hook at a
// flag-encoding CREATE2 address, factory, router), wires them, approves every
// priced pair asset from deployments/arbitrum-stock-quotes.json (Reality
// rTokens with Uniswap V3 USDC pools), and renounces factory ownership so
// only the immutable ADMIN keeps control. Resumable like the mainnet script:
// HOOK/FACTORY/ROUTER skip deploys already done, QUOTES=0 skips approvals,
// RENOUNCE=0 keeps ownership for now.
//
//   HARDHAT_CONFIG=hardhat.config.size.ts ROBINHOOD_RPC_URL=https://arb1.arbitrum.io/rpc \
//   ROBINHOOD_CHAIN_ID=42161 PRIVATE_KEY=... ADMIN=0x5DdDEa56774f01fc9d207BBD7B7633596a2f4A0b \
//     npx hardhat run scripts/deploy-stockpad-arb.ts --network robinhood
import { ethers } from "hardhat";
import fs from "node:fs";
import path from "node:path";

// Uniswap V4 PoolManager, WETH, SwapRouter02 (V3 routes) and Chainlink ETH/USD on Arbitrum One.
...
async function main() {
  ...
  const depFile = process.env.DEPLOY_FILE ?? path.join(__dirname, "..", "deployments", "arbitrum-stockpad.json");
  ...
  // 4. Pair assets: WETH priced by Chainlink, the rTokens at their pool price.
  if (process.env.QUOTES !== "0") {
    ...
    const quotes = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "deployments", "arbitrum-stock-quotes.json"), "utf8")).quotes as { symbol: string; address: string; usd: number }[];
  ...
  Object.assign(dep, {
    ...
    uniswap: { poolManager: POOL_MANAGER, weth: WETH, swapRouter02: ROUTER02, stateView: "0x76Fd297e2D437cd7f76d50F01AfE6160f86e9990" },
