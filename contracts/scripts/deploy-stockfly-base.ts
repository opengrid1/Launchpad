...
async function main() {
  ...
  const factory = await (await ethers.getContractFactory("StockFlyFactory")).deploy(
    signer.address, admin, POOL_MANAGER, hookAddr, WETH, B20_FACTORY, { nonce: base + 1 },
  );
  ...
  // Curate the known stock pairs (open registry means any token works too).
  ...
  writeFileSync(join(__dirname, "../deployments/base-stockfly.json"), JSON.stringify(out, null, 2));
