...
// Deploy the koi.fun V3 launch model to Base mainnet:
//   - fixed 2% trade tax (no per-coin choice, no platform cut)
//   - every trade splits 50% to the coin's holder-reward vault / 50% creator
//   - creator fee routes to the launch's feeRecipient (defaults to launcher)
//   - atomic dev buy, protocolAdmin collect/unwind retained from V2
//
...
//   ADMIN=0x... KEEPER=0x... \
//   npx hardhat run scripts/deploy-stockfly-v3-base.ts --network robinhood
//
// Base mainnet
...
async function main() {
  ...
  for (const pair of STOCK_PAIRS) {
    await (await factory.listPair(pair)).wait();
    console.log("listed pair:", pair);
  }

  await (await factory.renounceOwnership()).wait();
  console.log("factory ownership renounced (protocolAdmin retains collect/admin)");

  const startBlock = await ethers.provider.getBlockNumber();
  const out = {
    chainId: 8453,
    model: "v3: fixed 2% tax, 50% holders / 50% creator, no platform cut",
    admin,
    startBlock,
    contracts: {
  ...
  writeFileSync(join(__dirname, "../deployments/base-stockfly-v3.json"), JSON.stringify(out, null, 2));
  console.log("saved deployments/base-stockfly-v3.json  startBlock:", startBlock);
}
