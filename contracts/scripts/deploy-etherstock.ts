...
// Deploys Etherstock on Ethereum mainnet: hook (CREATE2 at a flag-encoding
...
//     npx hardhat run scripts/deploy-etherstock.ts --network robinhood
//
// Then verify (needs contracts/.env.verify: EXPLORER_API_KEY / EXPLORER_API_URL / EXPLORER_BROWSER_URL):
//   npx hardhat verify --network robinhood <hook> <poolManager> <admin>   (etc., printed at the end)
...
const BURN_BPS = 3000; // auto buyback and burn
...
async function main() {
  ...
  // 2. Token deployer, then the factory (deployer owns it until renounce; admin is immutable).
  ...
  const td = await ethers.getContractAt("EtherStockTokenDeployer", tdAddr);
  let factoryAddr: string = process.env.FACTORY ?? dep.contracts.factory ?? "";
  if (!factoryAddr) {
    const f = await (await ethers.getContractFactory("EtherStockFactory")).deploy(deployer.address, admin, POOL_MANAGER, hookAddr, tdAddr, WETH, price, TAX_BPS, CREATOR_BPS, BURN_BPS);
  ...
  const factory = await ethers.getContractAt("EtherStockFactory", factoryAddr);
  ...
  if (!routerAddr) {
    const r = await (await ethers.getContractFactory("EtherStockRouter")).deploy(POOL_MANAGER, factoryAddr, WETH, ROUTER02);
  ...
  Object.assign(dep, {
    ...
    fees: { taxBps: TAX_BPS, creatorBps: CREATOR_BPS, burnBps: BURN_BPS, platformBps: 10000 - CREATOR_BPS - BURN_BPS },
  ...
  console.log(`  npx hardhat verify --network robinhood ${hookAddr} ${POOL_MANAGER} ${admin}`);
  console.log(`  npx hardhat verify --network robinhood ${tdAddr}`);
  console.log(`  npx hardhat verify --network robinhood ${factoryAddr} ${deployer.address} ${admin} ${POOL_MANAGER} ${hookAddr} ${tdAddr} ${WETH} ${price} ${TAX_BPS} ${CREATOR_BPS} ${BURN_BPS}`);
  console.log(`  npx hardhat verify --network robinhood ${routerAddr} ${POOL_MANAGER} ${factoryAddr} ${WETH} ${ROUTER02}`);
}
