...
contract AlicornToken is ERC20, ReentrancyGuard {
    ...
    ///      be holding the pair as a V4 claim (see AlicornHook). Pull it in
    ///      before paying out.
    function _ensure(uint256 amount) private {
        if (IERC20(pairAsset).balanceOf(address(this)) < amount && hook != address(0)) IAlicornHook(hook).flush(address(this));
    }
