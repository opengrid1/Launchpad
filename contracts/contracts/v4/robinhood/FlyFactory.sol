...
import {FlyToken} from "./FlyToken.sol";
import {FlywheelHook} from "./FlywheelHook.sol";
...
contract FlyFactory is Ownable, ReentrancyGuard, IUnlockCallback {
    ...
    constructor(address owner_, address protocolAdmin_, IPoolManager poolManager_, FlywheelHook hook_, address weth_) Ownable(owner_) {
        weth = weth_;
        ...
        returns (address token, bytes32 poolId)
    {
        ...
        FlyToken qt = new FlyToken{salt: salt}(
            p.name, p.symbol, p.metadataURI, TOTAL_SUPPLY, msg.sender, address(this)
        );
        token = address(qt);
