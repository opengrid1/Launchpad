...
interface IDiamondWall {
    function creditWall(address token, uint256 amount) external;
}

interface IDiamondCoin {
    function sweepJeetPot() external returns (uint256);
    function distributeRewards(uint256 amount) external;
}

contract DiamondHook is BaseHook, ReentrancyGuard, IUnlockCallback {
    ...
    function _harvest(address token, uint256 minPairOut) private {
        ...
        // Diamond curve: pull the coin the early sellers paid; it converts to
        // WETH alongside the fee buckets and goes straight to the holders.
        uint256 jeet = IDiamondCoin(token).sweepJeetPot();
        uint256 holdersOut;
        if (tf + tfs + jeet > 0) {
            tokenFees[token] = 0;
            tokenFeesSniper[token] = 0;
            uint256 inTotal = tf + tfs + jeet;
            uint256 out = _swap(c.poolKey, tokenCurrency, inTotal, minPairOut);
            // Pro-rate the converted WETH into base / sniper / holder shares.
            uint256 sniperOut = (out * tfs) / inTotal;
            holdersOut = (out * jeet) / inTotal;
            pairFeesSniper[token] += sniperOut;
            pairFees[token] += out - sniperOut - holdersOut;
        }

        uint256 base = pairFees[token];
        uint256 sniper = pairFeesSniper[token];
        if (base + sniper + holdersOut == 0) {
        ...
        if (toWall > 0) {
            ...
            IDiamondWall(factory).creditWall(token, toWall);
        }
        if (holdersOut > 0) {
            IERC20(c.pair).safeTransfer(token, holdersOut);
            IDiamondCoin(token).distributeRewards(holdersOut);
        }
