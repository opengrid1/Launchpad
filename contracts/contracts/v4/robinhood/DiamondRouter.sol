...
import {RhFactory} from "./RhFactory.sol";
...
/// @title DiamondRouter
/// @notice One-tap buy/sell for launched coins on the Robinhood-chain fork,
///         where a coin is paired against a stock or meme token (pair = reward).
///         A buyer holds only native ETH, so the router bridges the gap:
///
...
///         launched coin's own V4 pool via the PoolManager unlock.
contract DiamondRouter is IUnlockCallback, ReentrancyGuard {
    ...
    RhFactory public immutable factory;
    ...
    constructor(IPoolManager pm, RhFactory factory_, address weth_, ISwapRouterV3 v3Router_) {
        poolManager = pm;
        factory = factory_;
    ...
    {
        address pair = _pairOf(coin);
        // Diamond coins skim the jeet tax on the way in, so sell whatever
        // actually arrived (fee-on-transfer aware).
        uint256 beforeBal = IERC20(coin).balanceOf(address(this));
        IERC20(coin).safeTransferFrom(msg.sender, address(this), amountIn);
        uint256 received = IERC20(coin).balanceOf(address(this)) - beforeBal;

        // 1. coin -> pair via the coin's own V4 pool.
        uint256 pairAmount = _v4Swap(coin, pair, coin, received);
