...
import {StockFeeHookV3 as StockFeeHook} from "./StockFeeHookV3.sol";
import {RewardVaultDeployer} from "./RewardVaultDeployer.sol";
import {IB20FactoryMin, IB20TokenMin} from "./IB20Min.sol";

/// @title StockFlyFactoryV2
...
contract StockFlyFactoryV3 is Ownable, ReentrancyGuard, IUnlockCallback {
    ...
    /// @notice Flat, fixed trade tax for every V3 launch: 2%. There is no
    ///         per-coin tax choice, no sniper premium and no platform cut —
    ///         the whole 2% is split 50% holders / 50% creator on each trade.
    uint16 public constant FIXED_TAX_BPS = 200;

    IPoolManager public immutable poolManager;
    StockFeeHook public immutable hook;
    RewardVaultDeployer public immutable vaultDeployer;
    address public immutable keeper;

    /// @notice The holder-reward vault minted for each coin at launch.
    mapping(address token => address) public rewardVaultOf;
    /// @notice Where each coin's 50% creator share of the trade tax is pushed.
    ///         Only this wallet sees and can claim the creator fees.
    ...
    /// @dev Open by default: any Base token (WETH, B-20 stock, wrapped
    ///      equity) can be a pair. protocolAdmin can still curate via the
    ///      allowlist and flip this off to lock it down.
    ...
    struct LaunchParams {
        ...
        string metadataURI;
        address pair;
        /// @dev Creator's fee destination — where the 50% creator share of the
        ///      trade tax is pushed each trade. Only this wallet sees and can
        ///      claim it. Defaults to msg.sender (the launcher) when zero.
    ...
    event RewardVault(address indexed token, address indexed vault, address stock);
        ...
        returns (address token, bytes32 poolId)
    {
        ...
        // Mint this coin's holder-reward vault and split the whole 2% tax
        // 50/50 between the creator and the vault ("hold coin, earn stock").
        // No burn, no auto-liquidity, no platform cut: the entire tax is
        // allocatable to these two payees.
        address vault = vaultDeployer.deploy(poolManager, token, p.pair, keeper, key, tokenIsCurrency0);
        rewardVaultOf[token] = vault;
        address feeTo = p.feeRecipient == address(0) ? msg.sender : p.feeRecipient;
        feeRecipientOf[token] = feeTo;
        StockFeeHook.Payee[] memory payees = new StockFeeHook.Payee[](2);
        payees[0] = StockFeeHook.Payee({to: feeTo, shareBps: 5_000});
        payees[1] = StockFeeHook.Payee({to: vault, shareBps: 5_000});
        ...
        emit RewardVault(token, vault, p.pair);

        // Optional dev buy, atomic with the launch so nobody can trade first.
        // The hook charges the flat 2% (no sniper premium) because the swap
    ...
    /// @notice Reconstruct a launched coin's PoolKey from its stored listing.
    ...
    /// @notice Authoritative spot read from the pool for a launched coin.
    ///         Returns the live sqrtPriceX96, which side the coin sits on, and
    ///         the pair token's decimals — everything the frontend needs to
    ///         derive an exact, decimals-correct token price (coin is always
    ///         18 decimals). Multiply the resulting pair-per-token by the pair
    ///         token's USD price for the USD spot; times 1e9 supply for mcap.
    ///         This removes the 8-vs-18 decimals guesswork from off-chain code.
        ...
        returns (uint160 sqrtPriceX96, bool tokenIsCurrency0, uint8 pairDecimals)
    {
        ...
        pairDecimals = _tryDecimals(pair);
