...
contract StockFlyFactoryV2 is Ownable, ReentrancyGuard, IUnlockCallback {
        ...
        returns (address token, bytes32 poolId)
    {
        ...
        if (p.taxBps == 0 || p.taxBps > MAX_TAX_BPS) revert InvalidParams();
        if (uint256(p.burnBps) + p.liquidityBps > 10_000) revert InvalidParams();
        ...
        // Mint this coin's holder-reward vault and split the payee share
        ...
        uint16 payShare = uint16(10_000 - p.burnBps - p.liquidityBps);
        uint16 holderShare = payShare / 2;
        StockFeeHook.Payee[] memory payees = new StockFeeHook.Payee[](2);
        payees[0] = StockFeeHook.Payee({to: msg.sender, shareBps: payShare - holderShare});
        payees[1] = StockFeeHook.Payee({to: vault, shareBps: holderShare});
        hook.configurePool(key, p.taxBps, p.burnBps, p.liquidityBps, tokenIsCurrency0, payees);
