...
contract StockFlyFactory is Ownable, ReentrancyGuard, IUnlockCallback {
    ...
    constructor(address owner_, address protocolAdmin_, IPoolManager poolManager_, StockFeeHook hook_, address weth_, IB20FactoryMin b20Factory_) Ownable(owner_) {
        weth = weth_;
        ...
        returns (address token, bytes32 poolId)
    {
        ...
        StockFeeHook.Payee[] memory payees = new StockFeeHook.Payee[](1);
        payees[0] = StockFeeHook.Payee({to: msg.sender, shareBps: uint16(10_000 - p.burnBps - p.liquidityBps)});
