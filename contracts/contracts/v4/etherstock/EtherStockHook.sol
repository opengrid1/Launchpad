...
interface IEtherStockCoin {
    function accrue(uint256 fee, uint256 extra) external;
}

/// @title EtherStockHook
/// @notice Fee engine for Etherstock's Uniswap V4 pools. Every swap
...
///         platform on the spot and sets the burn share aside; the coin's own
///         buyback swaps (sender == the coin) pay no fee.
///
...
contract EtherStockHook is BaseHook, IUnlockCallback {
        ...
        returns (bytes4, BeforeSwapDelta, uint24)
    {
        ...
        if (!c.registered || c.taxBps == 0 || sender == c.token) return (BaseHook.beforeSwap.selector, toBeforeSwapDelta(0, 0), 0);
        ...
        returns (bytes4, int128)
    {
        PoolId id = key.toId();
        PoolConfig storage c = _config[id];
        if (!c.registered || c.taxBps == 0 || sender == c.token) return (BaseHook.afterSwap.selector, 0);
        ...
        IEtherStockCoin(c.token).accrue(baseFee, fee - baseFee);
