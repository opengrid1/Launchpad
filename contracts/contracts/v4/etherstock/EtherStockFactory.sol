...
import {EtherStockHook} from "./EtherStockHook.sol";
import {EtherStockTokenDeployer} from "./EtherStockTokenDeployer.sol";
...
/// @title EtherStockFactory
/// @notice Etherstock: one-transaction launcher on Ethereum mainnet / Uniswap V4. A coin
...
///         same block; the EtherStockHook takes the fee on every swap and the
///         coin buys back and burns itself with its share, automatically.
///
...
contract EtherStockFactory is ReentrancyGuard, IUnlockCallback {
    ...
    /// @notice Fee split of every trade fee, bps: creator / burn / platform.
    uint16 public immutable CREATOR_BPS;
    uint16 public immutable BURN_BPS;
    /// @notice A coin's burn reserve triggers a buyback once it is worth this much (USD, 8 dp).
    uint256 public constant BUYBACK_MIN_USD_8 = 25 * 1e8;
    ...
    EtherStockHook public immutable hook;
    /// @notice Creates the coins (keeps this contract under the size limit).
    EtherStockTokenDeployer public immutable tokenDeployer;
    ...
    constructor(
        ...
        EtherStockHook hook_,
        EtherStockTokenDeployer tokenDeployer_,
        address weth_,
        uint64 ethUsd8_,
        uint16 taxBps_,
        uint16 creatorBps_,
        ...
        if (ethUsd8_ == 0 || taxBps_ == 0 || taxBps_ > 1_000 || uint256(creatorBps_) + burnBps_ > 10_000) revert InvalidParams();
        TAX_BPS = taxBps_;
        admin = admin_;
        feeRecipient = admin_;
        ...
        returns (address token, bytes32 poolId, PoolKey memory key, bool tokenIsCurrency0, address pair)
    {
        ...
        token = tokenDeployer.deploy(salt, p.name, p.symbol, p.metadataURI, TOTAL_SUPPLY, msg.sender, pair, poolManager, CREATOR_BPS, BURN_BPS);
        EtherStockToken t = EtherStockToken(token);

        (key, tokenIsCurrency0) = _key(token, pair);

        address[] memory ex = new address[](1);
        ex[0] = address(poolManager);
        // Buyback threshold in pair units: $25 at the pair's USD price.
        t.initHook(address(hook), converter, key, tokenIsCurrency0, Math.mulDiv(BUYBACK_MIN_USD_8, 1e18, pairUsd8), ex);
    ...
    function pushPlatformFees(address[] calldata tokens) external {
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            EtherStockToken(tokens[i]).claimPlatformFees();
        }
    }
